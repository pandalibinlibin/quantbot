...
class FactorIncrementalManager:
    ...
    def analyze_computation_strategy(
        ...
        try:
            ...
            if force_full:
                ...
                strategy["computation_ranges"] = [{"start": target_start, "end": target_end}]
            ...
            data_analysis = self._analyze_existing_data(existing_data, target_start, target_end)
            ...
            if data_analysis["coverage_ratio"] < 0.8:  # Less than 80% coverage
                strategy["strategy"] = "full"
                strategy["computation_ranges"] = [{"start": target_start, "end": target_end}]
                strategy["reasons"].append(f"Low data coverage: {data_analysis['coverage_ratio']:.2%}")
            
            elif data_analysis["has_gaps"]:
                strategy["strategy"] = "incremental"
                strategy["computation_ranges"] = data_analysis["gap_ranges"] + data_analysis["extension_ranges"]
                strategy["reasons"].append("Data gaps detected, incremental computation needed")
            ...
            logger.info(f"Strategy for '{factor_name}': {strategy['strategy']} - {', '.join(strategy['reasons'])}")
            return strategy
            
        except Exception as e:
            logger.error(f"Failed to analyze computation strategy for '{factor_name}': {e}")
    ...
    def _analyze_existing_data(
        ...
        try:
            ...
            # Calculate coverage
            expected_in_range = [t for t in expected_times if existing_start <= t <= existing_end]
            covered_times = [t for t in expected_in_range if t in existing_times]
            coverage_ratio = len(covered_times) / len(expected_in_range) if expected_in_range else 0
            
            # Detect gaps
            missing_times = [t for t in expected_in_range if t not in existing_times]
            gap_ranges = self._group_consecutive_dates(missing_times) if missing_times else []
            
            # Check if extension is needed
            extension_ranges = []
            if target_start < existing_start:
                extension_ranges.append({"start": target_start, "end": existing_start - timedelta(days=1)})
            if target_end > existing_end:
                extension_ranges.append({"start": existing_end + timedelta(days=1), "end": target_end})
            ...
            return {
                "existing_range": None,
                "data_shape": existing_data.shape if existing_data is not None else (0, 0),
                "coverage_ratio": 0.0,
                "has_gaps": True,
                "gap_ranges": [],
    ...
    def _generate_expected_times(
        ...
        try:
            if self.freq == "day":
                ...
                # Generate minute-level timestamps (trading hours only)
                return pd.date_range(start=start_time, end=end_time, freq='1min').tolist()
            else:
                # Default to daily frequency
                return pd.date_range(start=start_time, end=end_time, freq='D').tolist()
                
        except Exception as e:
            logger.error(f"Failed to generate expected times: {e}")
            return []
    
    def _group_consecutive_dates(self, dates: List[pd.Timestamp]) -> List[Dict[str, pd.Timestamp]]:
        """
        ...
        try:
            with Session(engine) as session:
                ...
                if factor:
                    ...
                    logger.info(f"Updated computation status for '{factor_name}' to {status}")
                    return True
                else:
            ...
            logger.error(f"Failed to update computation status for '{factor_name}': {e}")
            return False
    
    def get_computation_summary(self, factor_names: Optional[List[str]] = None) -> Dict[str, Any]:
        """
