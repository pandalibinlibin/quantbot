...
经过深入分析 Qlib 的最佳实践，我们决定对系统进行重大架构调整：

**核心认知**：

- Qlib 是一个**多因子选股系统**，专注于"买什么、买多少"
- Qlib **不具备择时能力**，无法决定"何时买、以什么价格买"
- 择时和执行应该委托给专业的执行系统（VeighNa、LEAN）

**调整目标**：

- 将 QuantBot 定位为**纯选股系统**
- 移除 Paper Trading 功能（委托给执行层）
- 实现**指数增强策略**，输出目标持仓权重
- 移除分钟数据支持（执行层自己获取实时数据）
...
│  │              执行层 (外部系统)                        │   │
│  │                                                     │   │
│  │  VeighNa (A股)          LEAN/QuantConnect (美股)    │   │
│  │  ├─ 接收目标持仓         ├─ 接收目标持仓             │   │
│  │  ├─ 实时行情数据         ├─ 实时行情数据             │   │
│  │  ├─ 择时执行             ├─ 择时执行                 │   │
│  │  ├─ TWAP/VWAP           ├─ 执行算法                 │   │
│  │  └─ Paper/Live Trading  └─ Paper/Live Trading       │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
| 指数        | 代码      | 市场 | 执行系统 |
| ----------- | --------- | ---- | -------- |
| 沪深300     | csi300    | A股  | VeighNa  |
| 中证500     | csi500    | A股  | VeighNa  |
| 标普500     | sp500     | 美股 | LEAN     |
| 纳斯达克100 | nasdaq100 | 美股 | LEAN     |

### 配置参数

在 `system_config.yaml` 中新增：

```yaml
enhanced_indexing:
  enabled: true
  max_deviation: 0.02 # 单只股票最大偏离度（默认2%）
  min_weight: 0.001 # 最小权重阈值
  benchmark: "auto" # auto = 根据数据自动检测
```
...
- 从前端导航栏移除 Paper Trading 入口
- 保留后端 API 代码（未来可能需要）

**修改文件**:

- `frontend/src/routes/_layout.tsx` - 移除导航项

**验证方法**:

- 启动前端，确认导航栏中没有 Paper Trading 入口
- 确认其他页面正常工作

---

### Task 2: 实现简化版指数增强策略服务
...
```python
def enhanced_indexing_simple(signals, benchmark_weights, max_deviation=0.02):
    """
    简化版指数增强策略

    算法步骤：
    1. 获取指数成分股权重 (benchmark_weights)
    2. 标准化预测分数到 [-1, 1]
    3. 计算偏离量 = 分数 * 最大偏离度
    4. 计算目标权重 = 基准权重 + 偏离量
    5. 确保权重 >= 0
...
- `backend/app/services/enhanced_indexing_service.py`

**修改文件**:

- `backend/app/config/qlib/system_config.yaml` - 添加配置
...
```json
{
  ...
  "summary": {
    ...
    "underweight_count": 150,
    "generated_at": "2026-03-03T11:30:00"
  }
}
```
...
- `backend/app/services/online_serving_service.py` - 集成指数增强

**验证方法**:

- 调用 routine API，确认返回目标持仓
- 确认持仓数据正确持久化
...
```
主题: [QuantBot] 每日目标持仓 - 2026-03-03

基准指数: 沪深300
生成时间: 2026-03-03 11:30:00

目标持仓权重表:
┌─────────────────────────────────────────────────────────────┐
│ 股票      指数权重    预测分数    调整后权重    偏离        │
├─────────────────────────────────────────────────────────────┤
│ 贵州茅台   6.0%       0.0312     7.2%         +1.2% (超配) │
│ 宁德时代   4.5%       0.0089     4.8%         +0.3% (超配) │
│ 招商银行   3.2%      -0.0156     2.5%         -0.7% (低配) │
│ ...                                                        │
└─────────────────────────────────────────────────────────────┘
...
- `backend/app/services/email_service.py` - 更新邮件模板
...
- `docs/integration/veighna_integration.md`
- `docs/integration/lean_integration.md`

**文档内容**:

1. 系统安装和配置
2. 如何获取 QuantBot 目标持仓
3. 如何导入到执行系统
4. Paper Trading 设置
5. Live Trading 注意事项
...
```
Task 1 (隐藏 Paper Trading)
    ↓
Task 2 (指数增强服务)
    ↓
Task 3 (Routine API 修改)
    ↓
Task 4 (邮件通知更新)
    ↓
Task 5 (移除分钟数据)
...
| Task 3 | 实现指数增强策略服务          | 🔲 待开始 | -          |
| Task 4 | 修改 Routine API 集成指数增强 | 🔲 待开始 | -          |
| Task 5 | 更新邮件通知                  | 🔲 待开始 | -          |
| Task 6 | 移除分钟数据支持              | 🔲 待开始 | -          |
| Task 7 | 创建 VeighNa 集成文档         | 🔲 待开始 | -          |
| Task 8 | 创建 LEAN 集成文档            | 🔲 待开始 | -          |
...
Qlib 提供了强化学习（RL）功能用于订单执行优化，包括 PPO、OPDS 等算法。这些算法可以学习最优的订单拆分和执行时机，以最小化市场冲击成本和滑点。

### 当前方案

目前执行层委托给 VeighNa（A股）和 LEAN（美股），使用它们内置的执行算法（TWAP/VWAP）。

### 未来结合方案

当资金规模较大（>1亿）时，可以考虑将 Qlib RL 与执行系统结合：

```
┌─────────────────────────────────────────────────────────────┐
│  Qlib RL + VeighNa/LEAN 结合方案                            │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  1. 用 Qlib RL 训练执行策略模型                              │
│     └─ 输出：训练好的 PPO/OPDS 模型权重                      │
│                                                             │
│  2. 将模型导出到 VeighNa/LEAN                                │
│     └─ 需要开发：模型格式转换、推理引擎集成                  │
│                                                             │
│  3. VeighNa/LEAN 使用模型做实时决策                          │
│     └─ 实时行情 → 模型推理 → 下单                           │
│                                                             │
│  复杂度：高                                                  │
│  收益：对于大资金（>1亿）有意义，小资金收益有限              │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
| 资金规模   | 推荐执行策略          | 原因                        |
| ---------- | --------------------- | --------------------------- |
| < 1000万   | 简单 TWAP/VWAP        | 冲击成本可忽略              |
| 1000万-1亿 | VeighNa/LEAN 内置算法 | 足够好                      |
| > 1亿      | 考虑 RL 优化          | 冲击成本显著，RL 优化有价值 |
